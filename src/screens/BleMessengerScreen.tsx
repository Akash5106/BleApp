...
export const BleMessengerScreen = () => {
  ...
  const sendNewMessage = async (text: string, destId: string) => {
    const msg = {
    ...
    const conn = await scanAndConnectOnce();
    if (!conn) return;
  ...
  const forwardMessage = async (msg: any) => {
    if (msg.ttl <= 0) return;

    msg.ttl -= 1;

    if (!msg.visited.includes(myId)) {
      msg.visited.push(myId);
    }
  ...
  const scanAndConnectOnce = (): Promise<Device | null> => {
    return new Promise(resolve => {
      let resolved = false;

      manager.startDeviceScan([SERVICE_UUID], null, async (error, device) => {
        if (error || !device || resolved) return;
  ...
  useEffect(() => {
    ...
    const sub = eventEmitter.addListener('onMessageReceived', async event => {
      ...
      // 1️⃣ Handshake logic (KEEP AS IS for now)
        ...
        if (activeConnection) {
          ...
          try {
            await activeConnection.writeCharacteristicWithResponseForService(
              SERVICE_UUID,
              CHAR_UUID,
              Buffer.from(myAuth).toString('base64'),
            );
          } catch (e) {
      ...
      // 2️⃣ FLOODING / MESH LOGIC (NEW)
      try {
        const msg = JSON.parse(data);

        // deduplication (per phone)
        ...
        if (msg.dest === myId) {
          ...
          setMessages(prev => [...prev, { sender: 'peer', text: msg.body }]);
          return;
        }

        // forward automatically (NO user action)
        await forwardMessage(msg);
      } catch (e) {
        // 3️⃣ fallback: plain text (old behavior)
      ...
      if (device) {
        setDevices(prev => {
          ...
          return [
            ...
            {
              id: device.id,
              name: device.name || device.localName || 'BleChat User',
            },
          ];
        });
      }
  ...
  const connectAndVerify = async (device: any) => {
    try {
      ...
      Alert.alert(
        'Error',
        "Peer not reachable. Ensure they clicked 'Go Visible'.",
      );
    }
  };
  ...
  const sendMessage = async () => {
    ...
    try {
      await activeConnection.writeCharacteristicWithResponseForService(
        SERVICE_UUID,
        CHAR_UUID,
        Buffer.from(inputText).toString('base64'),
      );
    ...
    return (
      <View style={styles.mainContainer}>
        <View style={styles.topBar}>
          <Text style={styles.barTitle}>Hello, {myName}</Text>
        </View>
        <ScrollView contentContainerStyle={styles.scrollPad}>
          <TouchableOpacity
            style={[
              styles.card,
              { backgroundColor: isAdvertising ? '#C8E6C9' : '#E3F2FD' },
            ]}
            onPress={handleStartAdvertising}
          >
            <Text style={styles.cardTitle}>
              {isAdvertising ? '✅ You are Visible' : 'Go Visible'}
            </Text>
            <Text style={styles.cardSub}>
              Others can find you to start a chat
            </Text>
          </TouchableOpacity>
          ...
          >
            {isScanning ? (
              ...
              <Text style={styles.btnText}>Find Nearby People</Text>
            )}
          </TouchableOpacity>
      ...
      >
        {messages.map((m, i) => (
          <View
            ...
            style={[
              styles.bubble,
              m.sender === 'me' ? styles.myBubble : styles.peerBubble,
            ]}
          >
            <Text
              style={[styles.msgText, m.sender === 'me' && { color: '#fff' }]}
            >
              {m.text}
            </Text>
          </View>
